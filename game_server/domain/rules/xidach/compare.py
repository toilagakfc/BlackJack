from .hand_type import *
from .hand_value import *
def compare(player_hand, dealer_hand):
        p_point = calc_point(player_hand)
        d_point = calc_point(dealer_hand)
        ...
        # 2. Xì bàng
        if is_xi_bang(player_hand) and not is_xi_bang(dealer_hand):
            return ("WIN", "PLAYER_XI_BANG", 2)

        if is_xi_bang(dealer_hand) and not is_xi_bang(player_hand):
            return ("LOSE", "DEALER_XI_BANG", 2)

        if is_xi_bang(player_hand) and is_xi_bang(dealer_hand):
            return ("DRAW", "BOTH_XI_BANG", 0)

        # 3. Ngũ linh
        if is_ngu_linh(player_hand):
            if is_ngu_linh(dealer_hand):
                if p_point < d_point:
